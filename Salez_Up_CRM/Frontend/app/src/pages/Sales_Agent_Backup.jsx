...
const UpdateModal = ({ isOpen, onClose, data }) => {
  ...
  useEffect(() => {
    ...
    axios.get('http://127.0.0.1:8000/api/team_leaders')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`http://127.0.0.1:8000/api/sales_agents_update/${data.id}`, updatedData)
      .then(response => {
...
const SalesAgents = () => {
  ...
  const handleDelete = (id) => {
    axios.delete(`http://127.0.0.1:8000/api/sales_agents_delete/${id}`)
      .then(response => {
        // Update state or refetch data after successful delete
        fetch('http://127.0.0.1:8000/api/sales_agents/34')
          .then(response => response.json())
          .then(data => {
            setTeams(data);
